...
export function TaskLinks({ actionId, waitingItemId, wbsNodeId }: TaskLinksProps) {
  ...
  const handleEditSave = async () => {
    ...
    if (url.length > MAX_URL || editLabel.length > MAX_LABEL) {
      toast.error("Too long");
      return;
    }
    const prev = links;
    setLinks((p) => p.map((l) => (l.id === editingId ? { ...l, url, label: editLabel.trim() } : l)));
    cancelEdit();
    const { error } = await supabase
      .from("task_links")
      .update({ url, label: editLabel.trim() })
      .eq("id", editingId);
    if (error) {
      setLinks(prev);
      toast.error("Could not save link", { description: error.message });
              ...
              return (
                <li key={link.id} className="rounded-lg border border-primary/40 bg-card p-3 space-y-2">
                  ...
                  <div className="flex gap-1.5">
                    <Button size="sm" className="h-7" onClick={() => void handleEditSave()} disabled={!editUrl.trim()}>
      ...
      {showAdd && (
        <div className="rounded-lg border border-primary/40 bg-card p-3 space-y-2">
          <Input
            ...
            placeholder="Label (optional, e.g. 'Design spec')"
            maxLength={MAX_LABEL}
            className="h-8 text-sm"
          />
          <Input
          ...
          />
          <div className="flex gap-1.5">
            <Button size="sm" className="h-7" onClick={() => void handleAdd()} disabled={!newUrl.trim() || submitting}>
