...
import { WorkPackage, DependencyType, Dependency } from "@/lib/types";
...
import { GanttChart, Plus, Trash2, AlertTriangle, Info } from "lucide-react";
...
export default function GanttPage() {
  const { workPackages, projects } = useFilteredData();
  ...
  const groupedWPs = useMemo(() => {
    const groups: { projectName: string; wps: WorkPackage[] }[] = [];
    ...
    for (const [name, wps] of projectMap) {
      groups.push({ projectName: name, wps });
    }
    return groups;
  }, [workPackages]);

  const flatWPs = groupedWPs.flatMap((g) => g.wps);
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            <GanttChart className="h-6 w-6 text-primary" /> Gantt Chart
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            {flatWPs.length} work packages · {criticalCount} on critical path
          </p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch checked={showCritical} onCheckedChange={setShowCritical} id="critical" />
            <Label htmlFor="critical" className="text-sm cursor-pointer">Highlight Critical Path</Label>
          </div>
          <div className="flex items-center gap-3 text-xs">
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-primary" /> Normal</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-[hsl(var(--rag-red))]" /> Critical</span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-sm bg-muted-foreground/30" /> No dates</span>
          </div>
        </div>
      </div>
      ...
      {flatWPs.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            ...
            <p>No work packages found. Create work packages with start and due dates to see the Gantt chart.</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="overflow-hidden">
          <div className="flex">
            ...
            <div className="flex-shrink-0 border-r bg-card z-10" style={{ width: LABEL_WIDTH }}>
              ...
              {groupedWPs.map((group) => (
                <div key={group.projectName}>
                  {groupedWPs.length > 1 && (
                    <div className="px-3 py-1 text-[10px] font-semibold uppercase text-muted-foreground bg-muted/50 tracking-wider border-b">
                      {group.projectName}
                    </div>
                  )}
                  {group.wps.map((wp) => {
                    ...
                    return (
                      <div
                        key={wp.id}
                        className={cn(
                          "flex items-center gap-2 px-3 border-b transition-colors cursor-pointer hover:bg-accent/50",
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{wp.workPackage}</p>
                          <p className="text-[10px] text-muted-foreground truncate">{wp.wpLead || "No lead"}</p>
                        </div>
                        <RAGBadge status={wp.ragStatus} />
                        {isCritical && (
                          <Tooltip>
                            <TooltipTrigger>
                              <AlertTriangle className="h-3.5 w-3.5 text-[hsl(var(--rag-red))]" />
                            </TooltipTrigger>
                            <TooltipContent>Critical path · Slack: {node?.slack ?? 0} days</TooltipContent>
                          </Tooltip>
                        )}
                      </div>
                    );
            ...
            <div className="flex-1 overflow-x-auto" ref={scrollRef}>
              <div style={{ width: chartWidth, minWidth: "100%" }}>
                ...
                {/* Bars */}
                <svg width={chartWidth} height={flatWPs.length * ROW_HEIGHT + 60} className="overflow-visible">
                  {/* Today line */}
                  {(() => {
                    ...
                    if (todayOff >= 0 && todayOff <= totalDays) {
                      return (
                        <line
                          ...
                          y2={flatWPs.length * ROW_HEIGHT + 60}
                  ...
                  {weeks.map((w, i) => {
                    ...
                    return <line key={i} x1={x} y1={0} x2={x} y2={flatWPs.length * ROW_HEIGHT + 60} stroke="hsl(var(--border))" strokeWidth={0.5} />;
                  })}

                  {/* Bars */}
                  {flatWPs.map((wp, rowIdx) => {
                    if (!wp.startDate || !wp.dueDate) {
                      return (
                        <g key={wp.id}>
                          <rect
                            x={10}
                            y={rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2 - 4}
                            width={60}
                            height={8}
                            rx={4}
                            fill="hsl(var(--muted-foreground) / 0.2)"
                          />
                          <text x={80} y={rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2 + 3} fontSize={9} fill="hsl(var(--muted-foreground))">
                            No dates
                          </text>
                        </g>
                      );
                    ...
                    const y = rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2 - 10;
                    const node = cpmNodes.get(wp.id);
                    const isCritical = node?.isCritical && showCritical;
                  ...
                  {flatWPs.map((wp, rowIdx) =>
                    (wp.dependencies || []).map((dep) => {
                      const predIdx = flatWPs.findIndex((w) => w.id === dep.targetId);
                      const pred = flatWPs[predIdx];
                      if (predIdx < 0 || !pred) return null;
                      if (!pred.startDate || !pred.dueDate || !wp.startDate || !wp.dueDate) return null;

                      const predStartOff = dayOffset(pred.startDate);
                      const predEndOff = dayOffset(pred.dueDate);
                      const succStartOff = dayOffset(wp.startDate);
                      const succEndOff = dayOffset(wp.dueDate);

                      let fromX: number, fromY: number, toX: number, toY: number;

                      switch (dep.type) {
                        case "FS":
                          fromX = predEndOff * DAY_WIDTH;
                          fromY = predIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          toX = succStartOff * DAY_WIDTH;
                          toY = rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          break;
                        case "FF":
                          fromX = predEndOff * DAY_WIDTH;
                          fromY = predIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          toX = succEndOff * DAY_WIDTH;
                          toY = rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          break;
                        case "SS":
                          fromX = predStartOff * DAY_WIDTH;
                          fromY = predIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          toX = succStartOff * DAY_WIDTH;
                          toY = rowIdx * ROW_HEIGHT + ROW_HEIGHT / 2;
                          break;
