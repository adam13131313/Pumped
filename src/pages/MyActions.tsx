...
export default function MyActions() {
  ...
  const handleSave = (action: Action) => {
    if (editing) updateAction(action.id, action);
      ...
      addAction(action);
      clearFilters();
      const hasGlobalFilter = globalFilter.nodeId || globalFilter.unassigned;
      if (hasGlobalFilter) {
        // If the new action wouldn't appear under the current scope, clear it.
        const inSubtree = (() => {
          if (globalFilter.unassigned) return !action.wbsNodeId;
          if (!globalFilter.nodeId) return true;
          if (!action.wbsNodeId) return false;
          const path = nodePath(wbsNodes, action.wbsNodeId);
          return path.some((n) => n.id === globalFilter.nodeId);
        })();
        if (!inSubtree) {
          clearGlobalFilter();
  ...
  const getOrderedActions = useCallback((status: ActionStatus, colActions: Action[]) => {
  ...
  const handleReorder = useCallback((status: ActionStatus, fromIndex: number, toIndex: number, colActions: Action[]) => {
    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 rounded-lg border bg-accent/50 p-3">
          ...
          <Select onValueChange={(v) => handleBulkStatusChange(v as ActionStatus)}>
          ...
          <Select onValueChange={(v) => handleBulkPriorityChange(v as ActionPriority)}>
            <SelectTrigger className="h-8 w-[130px] text-xs">
      ...
      {allActions.length === 0 ? (
        ...
        <ListView
          ...
          wbsNodes={wbsNodes}
          onApplyNode={(id, nodeId) => updateAction(id, { wbsNodeId: nodeId })}
        />
      ) : (
        <KanbanView
          actions={actions}
      ...
      <ActionDialog
        ...
        onDelegate={(id, params) => { delegateAction(id, params); setEditing(null); }}
      />
    </div>
  );
}
...
function nodeLabel(wbsNodes: WbsNode[], nodeId: string | null): string {
  if (!nodeId) return "";
...
function ListView({
  actions, onEdit, selected, onToggle, onToggleAll, wbsNodes, onApplyNode,
}: {
  actions: Action[];
  ...
  onToggleAll: () => void;
  wbsNodes: WbsNode[];
  onApplyNode: (id: string, nodeId: string) => void;
}) {
  const todayIds = useAppStore((s) => s.todayIds);
  const addToday = useAppStore((s) => s.addToday);
  const removeToday = useAppStore((s) => s.removeToday);
  ...
  const workPackages = useMemo(
    () => wbsNodes.filter((n) => n.nodeType === "work_package" && !n.archivedAt),
    [wbsNodes],
  );
    ...
    try {
      const { data, error } = await supabase.functions.invoke("suggest-work-package", {
        body: {
          task: a.task,
          notes: a.notes,
          currentNodeId: a.wbsNodeId,
          workPackages: workPackages.map((w) => ({
            id: w.id,
            name: w.name,
            path: nodeLabel(wbsNodes, w.id),
          })),
        },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      if (!data?.suggestion?.nodeId) {
        toast.info(data?.reason || "No confident match found.");
        setSuggestions((p) => ({ ...p, [a.id]: null }));
      } else {
        setSuggestions((p) => ({ ...p, [a.id]: { nodeId: data.suggestion.nodeId, confidence: data.confidence, reason: data.reason } }));
      }
    } catch (e) {
  ...
  const acceptSuggestion = (a: Action, s: WPSuggestion) => {
    onApplyNode(a.id, s.nodeId);
    setSuggestions((p) => { const next = { ...p }; delete next[a.id]; return next; });
    toast.success(`Assigned to "${nodeLabel(wbsNodes, s.nodeId)}"`);
  };
                          ...
                          <div className="flex items-start gap-2 rounded-md border border-primary/30 bg-primary/5 p-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs">
                                ...
                                <span className="font-medium">{nodeLabel(wbsNodes, suggestion.nodeId)}</span>
...
function KanbanView({
  actions, wbsNodes, onStatusChange, onEdit, selected, onToggle, getOrderedActions, onReorder,
}: {
  actions: Action[];
  wbsNodes: WbsNode[];
  onStatusChange: (id: string, status: ActionStatus, dropIndex?: number) => void;
  onEdit: (a: Action) => void;
  selected: Set<string>;
  onToggle: (id: string) => void;
  getOrderedActions: (status: ActionStatus, colActions: Action[]) => Action[];
  onReorder: (status: ActionStatus, fromIndex: number, toIndex: number, colActions: Action[]) => void;
}) {
  const todayIds = useAppStore((s) => s.todayIds);
  const addToday = useAppStore((s) => s.addToday);
  const removeToday = useAppStore((s) => s.removeToday);

  const dragItemRef = useRef<{ id: string; sourceStatus: ActionStatus; sourceIndex: number } | null>(null);
  const [dropTarget, setDropTarget] = useState<{ status: ActionStatus; index: number } | null>(null);

  const handleDragStart = (id: string, status: ActionStatus, index: number) => {
    dragItemRef.current = { id, sourceStatus: status, sourceIndex: index };
  };

  const handleDragOverCard = (e: React.DragEvent, status: ActionStatus, index: number) => {
    e.preventDefault();
    setDropTarget({ status, index });
  };

  const handleDragOverColumn = (e: React.DragEvent, status: ActionStatus, colLength: number) => {
    e.preventDefault();
  ...
  const handleDrop = (status: ActionStatus, colActions: Action[]) => {
