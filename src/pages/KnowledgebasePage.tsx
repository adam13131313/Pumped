...
const sections = [
  ...
  { id: "projects", icon: FolderKanban, title: "Projects & Work Packages (WBS)", content: `Pumped organises work into a flexible WBS hierarchy: **Portfolio → Programme → Project → Work Package**. Portfolios can also contain sub-portfolios for super-portfolio rollups. Each Work Package can have its own lead, start/due dates, RAG status, and blockers; Projects can carry a high-level status (Active / On Hold / Complete).\n\n**RAG statuses:**\n- 🟢 **Green** — On track, no issues.\n- 🟡 **Amber** — Some risk or delay, being managed.\n- 🔴 **Red** — Off track, needs escalation.\n\n**Adding nodes:**\n- Use the **New Portfolio / Programme / Project** buttons at the top of the WBS page for top-level nodes.\n- Use **Add** on any existing card to create a child. Portfolios offer a dropdown with Sub-portfolio / Programme / Project / Work Package; other levels offer the next-down type.\n\n**Bulk import / export (CSV):**\nThree buttons on the WBS page: **Template** (download a starter CSV), **Export CSV** (your current hierarchy), **Import CSV** (file picker → preview → apply).\n\nCSV columns:\n\`\`\`\npath, node_type, description, project_status, start_date, due_date, rag_status, blockers, priority\n\`\`\`\n- **path** uses \` > \` to express hierarchy: \`My Portfolio > Programme A > Project 1 > WP X\`. Parent is path-minus-last-segment.\n- **node_type** accepts \`portfolio\`, \`programme\`, \`project\`, \`work_package\` for WBS nodes — and \`task\`, \`action\`, or \`activity\` for leaf tasks (which become **Actions** under the nearest work package).\n- **Sub-work-packages** (work_package nested under work_package) are accepted and converted into actions under the parent WP, with the sub-WP name preserved as a prefix on descendant task titles.\n- **Type-scoped fields on the wrong node type** (e.g. start_date on a programme) are silently dropped and surfaced as a single grouped warning rather than per-row errors.\n\nThe **import preview** shows what will be created/updated/skipped before anything is written. Errors and warnings are grouped by message with row counts and sample row numbers.` },
